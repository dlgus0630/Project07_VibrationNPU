...
1. FFT/NPU는 설명 가능한 소형 구조를 유지하고, 실제 데이터·run 분리 검증·안전 통합의 깊이를 기능 수보다 우선한다.
...
7. ARM은 센서 설정·데이터 이동·정책·로그를 담당하고, PL은 FFT/NPU와 hard real-time 제어를 담당한다.
...
16. 통합 시 모터 제어 주기와 안전 정지는 PL이 소유하며, 이상 판정은 연속 frame 조건과 latched fault를 거쳐 적용한다.
